...
void dsp_test_end_results(u32* addr_ptr, int pos, int add_mode, int type, int hold)
{
   int i;
   SCU_REG_PDA = 0;
   ...
   if (AUTOMATED == 0)
   {
      ...
      vdp_printf(&test_disp_font, 0 * 8, 25 * 8, 0xF, "%d", ct2);
   ...
   else
   {
      if (VERIFY)
      {
         ...
         for (i = 0; i < 65; i++)
         {
            ...
            {
               vdp_printf(&test_disp_font, 0 * 8, 26 * 8, 0xF, "%02X %08X should be %08X", i, data[i], results2[results2_pos][i]);
               vdp_printf(&test_disp_font, 0 * 8, 27 * 8, 0xF, "add %d type %d hold %d result pos %d", add_mode, type, hold, results2_pos);
               fail = 1;
...
void dsp_dma_to_dsp(u32 src_dest_addr, u32 bank, u32 add_mode, u8 length, int type, int addr_clear, int hold)
{
   ...
   dsp_test_end_results(addr_ptr, 0, add_mode, type, hold);
...
void dsp_dma_multiple_test()
{
   ...
   results2_pos = 0;
   test_dmas(0x25c01000);//vdp1 ram
   //results2_pos = 0;
   //test_dmas(0x25a01000);//sound ram
   //results2_pos = 0;
   //test_dmas(0x25e7f000);//vdp2 ram
   //results2_pos = 0;
   //test_dmas(0x22000000);//cart

   vdp_printf(&test_disp_font, 0 * 8, 25 * 8, 0xF, "pass");
...
void dsp_print_banks()
{
   ...
   for (i = 0; i < 24; i++)
   {
      vdp_printf(&test_disp_font, 0 * 8, (i + 1) * 8, 0xF, "%08X", banks[0][i]);
      vdp_printf(&test_disp_font, 9 * 8, (i + 1) * 8, 0xF, "%08X", banks[1][i]);
      vdp_printf(&test_disp_font, 18 * 8, (i + 1) * 8, 0xF, "%08X", banks[2][i]);
      vdp_printf(&test_disp_font, 27 * 8, (i + 1) * 8, 0xF, "%08X", banks[3][i]);
   }
}
