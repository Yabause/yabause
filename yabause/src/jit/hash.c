...
Hash *hash_new(int s)
{
   ...
   if ((h = (Hash *) malloc(sizeof(Hash))) == NULL)
      return h;

   h->size = s;
   h->hash = (int *) malloc(sizeof(int) * s);
   h->bloc = (u32 *) malloc(sizeof(u32) * s);
   h->list = (int *) malloc(sizeof(int) * s);
   memset(h->hash, -1, sizeof(int) * s);
   memset(h->bloc, 0xFF, sizeof(u32) * s);
...
void hash_remove_addr(Hash *h, u32 addr)
{
   u32 hash_addr = addr % h->size;
   u32 a;
   int i, j;
...
int hash_add_addr(Hash *h, u32 addr, int * ret)
{
   u32 hash_addr = addr % h->size;
   u32 bloc_addr;
   int i, j;
