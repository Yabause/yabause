...
u8 * CompileBlocks::CompileBlock(u32 pc,  u32 * ParentT = NULL)
{  
  compile_count_++;
  ...
  //if( g_Saturn->m_DbgMode == 0 )
  xlatPageInter(&g_CompleBlock[blockCount], ParentT);
  //else
  //  xlatPageDebug(&g_CompleBlock[blockCount], ParentT);

  return g_CompleBlock[blockCount].code;
}
...
#define addJumpTable(x,y,z)		jumps = (jmpTag *)(realloc((void*)jumps, sizeof(jmpTag)*((++jmp)+1))); \
                memset((void*)&jumps[jmp], 0, sizeof(jmpTag)); \
                jumps[jmp-1].ptr = x; \
                jumps[jmp-1].src = y; \
                jumps[jmp-1].dest = z;

#define addJumpTableF(x,y,z)	jmp++;\
                jpg.ptr = x; \
                jpg.src = y; \
                jpg.dest = z;

#define opdesc(op, y)	x86op_desc(x86_##op, &##op##_size, &##op##_src, &##op##_dest, &##op##_off1, &##op##_imm, &##op##_off3, y)
#define opNULL			x86op_desc(0,0,0,0,0,0,0,0)

#define PROLOGSIZE		     27    
#define EPILOGSIZE		      3
#define SEPERATORSIZE	     10
#define SEPERATORSIZE_NORMAL 31
#define SEPERATORSIZE_DEBUG  24
#define SEPERATORSIZE_DELAY  7
#define SEPERATORSIZE_DELAYD 14
...
#define SAFEPAGESIZE	MAXBLOCKSIZE - MAXINSTRSIZE - DELAYJUMPSIZE - SEPERATORSIZE_NORMAL - EPILOGSIZE
...
void seperator_deleay(void);

void seperator_d_normal(void);
void seperator_d_deleay(void);
...
x86op_desc asm_list[] =
{
  ...
  opdesc(STC_GBR,0xFF),
  opdesc(STC_VBR,0xFF),
  opdesc(STS_MACH,0xFF),
  opdesc(STS_MACL,0xFF),
  opdesc(STS_PR,0xFF),
  opdesc(TAS,0),         // 0x401b
  opdesc(STC_SR_MEM,0xFF),
  opdesc(STC_GBR_MEM,0xFF),
  opdesc(STC_VBR_MEM,0xFF),
  opdesc(STS_MACH_DEC,0xFF),
  opdesc(STS_MACL_DEC,0xFF),
  opdesc(STSMPR,0xFF),     // 0x4022
  opdesc(LDC_SR,0xFF),
  opdesc(LDCGBR,0xFF),
  opdesc(LDC_VBR,0xFF),
  opdesc(LDS_MACH,0xFF),
  opdesc(LDS_MACL,0xFF),
  opdesc(LDS_PR,0xFF),
  opdesc(JMP, 3),
  opdesc(JSR, 4),
  opdesc(LDC_SR_INC,0xFF),
  opdesc(LDC_GBR_INC,0xFF),
  opdesc(LDC_VBR_INC,0xFF),
  opdesc(LDS_MACH_INC,0xFF),
  opdesc(LDS_MACL_INC,0xFF),
  opdesc(LDS_PR_INC,0xFF),
  opdesc(BRAF,4),
  opdesc(BSRF,4),
  opdesc(ADD,0),
  opdesc(ADDC,0),
  opdesc(ADDV,0),  // 0x300F
  opdesc(AND,0),
  opdesc(CMP_EQ,0),
  opdesc(CMP_HS,0), 
  opdesc(CMP_GE,0),
  opdesc(CMP_HI,0),
  opdesc(CMP_GT,0), 
  opdesc(CMPSTR,0), // 0x200C
  opdesc(DIV1,0),   // 0x3004
  opdesc(DIV0S,0),  // 0x2007
  opdesc(DMULS,0),  // 0x300D
  opdesc(DMULU,0),  // 0x3005
  opdesc(EXTS_B,0),
  opdesc(EXTS_W,0),
  opdesc(EXTUB,0),
  opdesc(EXTU_W,0),
  opdesc(MOVR,0),  // 0x6003
  opdesc(MULL,0),  // 0x0007
  opdesc(MULS,0),  // 0x200f
  opdesc(MULU,0),  // 0x200e
  opdesc(NEG,0),
  opdesc(NEGC,0),
  opdesc(NOT,0),
  opdesc(OR,0),
  ...
  opdesc(MOVBL,0), // 6000
  opdesc(MOVWL,0), // 6001
  opdesc(MOVL_MEM_REG,0),
  opdesc(MAC_L,0),  // 0x000f
  opdesc(MAC_W,0),  // 0x400f
  opdesc(MOVBP,0),  // 6004
  opdesc(MOVWP,0),  // 6005
  opdesc(MOVLP,0),  // 6006
  opdesc(MOVBM,0),  // 0x2004,
  opdesc(MOVWM,0),  // 0x2005,
  opdesc(MOVLM,0),  // 0x2006
  opdesc(MOVBS0,0), // 0x0004
  opdesc(MOVWS0,0), // 0x0005
  opdesc(MOVLS0,0), // 0x0006
  opdesc(MOVBL0,0), // 0x000C
  opdesc(MOVWL0,0), // 0x000D 
  opdesc(MOVLL0,0), // 0x000E
  opdesc(MOVBL4,0), // 0x8400
  opdesc(MOVWL4,0), // 0x8500
  opdesc(MOVBS4,0), // 0x8000
  opdesc(MOVWS4,0), // 0x8100
  opdesc(MOVLS4,0), // 0x1000
  opdesc(MOVLL4,0), // 0x5000 
  opdesc(MOVBSG,0), // 0xC000
  opdesc(MOVWSG,0), // 0xc100
  opdesc(MOVLSG,0), // 0xC200
  opdesc(MOVBLG,0), // 0xC400
  opdesc(MOVWLG,0), // 0xC500
  opdesc(MOVLLG,0), // 0xC600
  opdesc(MOVA,0),
  opdesc(BF,1),
  opdesc(BF_S,3),
  opdesc(BT,1),
  ...
  opdesc(TSTI,0),  // C800
  opdesc(XORI,0),
  opdesc(TRAPA,5), // 0xc300
  opdesc(ADDI,0),
  opdesc(MOVI,0),
  opNULL
}; 

void CompileBlocks::xlatPageInter(Block *page, u32 * ParentT )
{
  ...
  while (ptr - startptr < SAFEPAGESIZE) {
    ...
    if (asm_list[i].func == 0) {
      ...
      //}
      memcpy((void*)ptr, (void*)seperator_normal, SEPERATORSIZE_NORMAL );
      instrSize[blockCount][count++] = SEPERATORSIZE_NORMAL ;
      ptr += SEPERATORSIZE_NORMAL;
      break;
    }
    ...
    if (asm_list[i].delay == 0) { 
      if( IntrruptCheck == 0 ) {
        ...
        memcpy((void*)(ptr + *(asm_list[i].size)), (void*)seperator_normal, SEPERATORSIZE_NORMAL );
        instrSize[blockCount][count++] = *(asm_list[i].size) + SEPERATORSIZE_NORMAL ;
        opcodePass(&asm_list[i], op, ptr); 
        ptr += *(asm_list[i].size) + SEPERATORSIZE_NORMAL;
      }else{
        memcpy((void*)ptr, (void*)(asm_list[i].func), *(asm_list[i].size));
        memcpy((void*)(ptr + *(asm_list[i].size)), (void*)seperator_deleay, SEPERATORSIZE_DELAY );
        instrSize[blockCount][count++] = *(asm_list[i].size) + SEPERATORSIZE_DELAY ;
        opcodePass(&asm_list[i], op, ptr); 
        ptr += *(asm_list[i].size) + SEPERATORSIZE_DELAY;
      }
      IntrruptCheck++;
    }
    ...
    // Normal Jump
      ...
      memcpy((void*)ptr, (void*)(asm_list[i].func), *(asm_list[i].size));
      memcpy((void*)(ptr + *(asm_list[i].size)), (void*)seperator_deleay, SEPERATORSIZE_DELAY );
      instrSize[blockCount][count++] = *(asm_list[i].size) + SEPERATORSIZE_DELAY + DELAYJUMPSIZE;
      opcodePass(&asm_list[i], op, ptr); 
      ptr += *(asm_list[i].size) + SEPERATORSIZE_DELAY + DELAYJUMPSIZE ;
    }
    ...
    // Jmp With Delay Operation
      ...
      //if(g_Saturn->IsCounting()) InstLog[j]++;
      ...
      memcpy((void*)(ptr + *(asm_list[j].size)), (void*)seperator_deleay, SEPERATORSIZE_DELAY);
      instrSize[blockCount][count++] = *(asm_list[j].size) + SEPERATORSIZE_DELAY + DELAYJUMPSIZE;
      opcodePass(&asm_list[j], temp, ptr);
      ptr += *(asm_list[j].size) + SEPERATORSIZE_DELAY + DELAYJUMPSIZE;
    }
    ...
    if(asm_list[i].delay == 1) { 
      addJumpTableF(ptr-DELAYJUMPSIZE, addr-2, addr+((signed char)(op&0xff)<<1)+2);
    }

    //offset3
    else if (asm_list[i].delay == 2) { 
      temp = (op&0xfff) << 1;
      if (temp & 0x1000) temp |= 0xfffff000;
      addJumpTableF(ptr-DELAYJUMPSIZE, addr-2, addr+((signed)(op&0xfff)<<1));
    }

    //immediate
    else if (asm_list[i].delay == 3) { 
      addJumpTableF(ptr-DELAYJUMPSIZE, addr-2, addr+((signed char)(op&0xff)<<1));
    }

    //indeterminate
    else if (asm_list[i].delay == 4) { 
      //forces a page flip
      addJumpTableF(ptr-DELAYJUMPSIZE, 0, 0);
    }

    // Trapa
    else if (asm_list[i].delay == 5) { 
      //forces a page flip
      addJumpTableF(ptr-DELAYJUMPSIZE, 0, 0);
    }
  ...
  page->e_addr = addr-2;
  if(jmp) memcpy((void*)jpg.ptr, (void*)PageFlip, DELAYJUMPSIZE);
...
void DynarecSh2::ExecuteCount( u32 Count ) {
  ...
  u32 targetcnt = m_pDynaSh2->SysReg[4] + Count;
...
void DynarecSh2::Execute(){
  ...
  //LOG("\n---dynaExecute %08X----\n", GET_PC() );
  ((dynaFunc)((void*)(pBlock)))(m_pDynaSh2); 
}
...
int DynarecSh2::InterruptRutine(u8 Vector, u8 level)
{
  if (((u32)level) > ((m_pDynaSh2->CtrlReg[0] >> 4) & 0x0F)) {
    ...
    m_pDynaSh2->GenReg[15] -= 4;
    memSetLong(m_pDynaSh2->GenReg[15], m_pDynaSh2->SysReg[3] + 2);
