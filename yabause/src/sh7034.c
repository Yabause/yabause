...
void onchip_write_byte(struct Onchip * regs, u32 addr, u8 data)
{
   ...
   else if (addr >= 0x5FFFF40 && addr <= 0x5FFFF7F)
   {
      ...
      if (addr == 0x5FFFF5e)
      {
         regs->dmac.channel[1].chcr = (regs->dmac.channel[0].chcr & 0xff) | data << 8;
         return;
      }
      if (addr == 0x5FFFF5F)
      {
         regs->dmac.channel[1].chcr = (regs->dmac.channel[0].chcr & 0xff00) | data & 0xfd;
         return;
      }
      if (addr == 0x5FFFF6e)
      {
         regs->dmac.channel[2].chcr = (regs->dmac.channel[0].chcr & 0xff) | data << 8;
         return;
      }
      if (addr == 0x5FFFF6F)
      {
         regs->dmac.channel[2].chcr = (regs->dmac.channel[0].chcr & 0xff00) | data & 0xfd;
         return;
      }
      if (addr == 0x5FFFF7e)
      {
         regs->dmac.channel[3].chcr = (regs->dmac.channel[0].chcr & 0xff) | data << 8;
         return;
      }
      if (addr == 0x5FFFF7F)
      {
         regs->dmac.channel[3].chcr = (regs->dmac.channel[0].chcr & 0xff00) | data & 0xfd;
         return;
      }
      //dmac
