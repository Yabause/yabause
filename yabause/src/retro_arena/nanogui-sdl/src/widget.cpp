...
Widget * Widget::getNearestWidget(Widget *parent, int dir ) {

  double dist = std::numeric_limits<double>::max();
  Widget * rtn = parent;

  Vector2i p = parent->absolutePosition();
  ...
  for (auto it = mChildren.rbegin(); it != mChildren.rend(); ++it) {
    ...
    if (child->visible() && parent != child) {
      ...
      case 1: {   // down
        double cpos = child->absolutePosition().y();
        double cdist = cpos- p.y();
        if (cdist > 0) {
          if (dist > cdist) {
            rtn = child;
            dist = cdist;
      ...
      case 2: {   // left
        double cdist = child->absolutePosition().x() - p.x();
        if (cdist < 0) {
          cdist = abs(cdist);
          if (dist > cdist) {
            rtn = child;
  ...
  //if (rtn != parent) {
  //  return rtn->getNearestWidget(parent, dir);
  //}
  return rtn;
}
