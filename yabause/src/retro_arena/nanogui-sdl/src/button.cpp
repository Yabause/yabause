    ...
    if (mIcon) {
        ...
        if (nvgIsFontIcon(mIcon)) {
          ...
          NVGpaint imgPaint = nvgImagePattern(ctx,
            iconPos.x(), iconPos.y() - ih / 2, iw, ih, 0, imageId, mEnabled ? 0.5f : 0.25f);
          nvgBeginPath(ctx);
          nvgRoundedRect(ctx, iconPos.x(), iconPos.y() - ih / 2, iw, ih, 5);
...
ImageButton::ImageButton(Widget *parent, const std::string &caption, int icon)
...
void ImageButton::draw(NVGcontext *ctx) {
  ...
  if (!mEnabled)
    ...
    if (mOnImageRequested)
      imageId = mOnImageRequested(mIcon, mPos.x(), mPos.y(), mSize.x(), mSize.y());
    else
      imageId = mIcon;
