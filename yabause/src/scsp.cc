...
Scsp::Scsp(SaturnMemory *v) : Dummy(0xFFF) {
  SDL_AudioSpec fmt;
  ...
  C68k_Set_Fetch(&C68K, 0x000000, 0x080000, (u32)basetruescspram);

  is68kOn = false;

  scsp_init(basetruescspram, &c68k_interrupt_handler, &scu_interrupt_handler);
  scsptiming1 = 0;
  scsptiming2 = 0;
  ...
  try {
    ...
    cerr << "SDL_InitSubSystem failed" << endl;
    exit(1);
  }
  ...
  try {
    ...
    cerr << "SDL_OpenAudio failed" << endl;
    exit(1);
  }
...
void Scsp::reset68k(void) {
  C68k_Reset(&C68K);
}

void Scsp::reset(void) {
  scsp_reset();
}
...
void Scsp::Convert32to16s(long *srcL, long *srcR, short *dst, unsigned long len) {
  unsigned long i;
...
unsigned char Scsp::getByte(unsigned long addr) {
   return scsp_r_b(addr);
}

void Scsp::setByte(unsigned long addr, unsigned char val) {
   scsp_w_b(addr, val);
}

unsigned short Scsp::getWord(unsigned long addr) {
   return scsp_r_w(addr);
}

void Scsp::setWord(unsigned long addr, unsigned short val) {
   scsp_w_w(addr, val);
}

unsigned long Scsp::getLong(unsigned long addr) {
