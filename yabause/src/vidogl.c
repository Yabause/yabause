...
//The polygon calculation seems not so good. A good test game is break point. All the lines are blinking and none is really thin and straight.
  ...
  if (gouraud == 1)
  {
    ...
    YglQuadGrowShading(&sprite, &texture, NULL, &c);
  }
  ...
  *texture.textdata = Vdp1ReadPolygonColor(&cmd);
  ...
  for (i=0; i<4; i++) {
    ...
    if (gouraud) {
      YglCacheQuadGrowShading(&sprite, col, &c);
    }
    else {
    ...
    if (back_pixel_data != NULL) {
      for (int i = 0; i < vdp2height; i++) {
        ...
        if (fixVdp2Regs->CCCTL & 0x2) {
          ...
          a = ((~fixVdp2Regs->CCRLB & 0x1F00) >> 8) * 255 / 31 ;
        }
...
static void Vdp2DrawRBG0(void)
{
  ...
  if ((fixVdp2Regs->CCCTL & 0xF000) == 0x9000) {
    info->alpha = (~fixVdp2Regs->CCRR & 0x1F) * 255 / 31;
    info->blendmode |= VDP2_CC_BLUR;
  }
