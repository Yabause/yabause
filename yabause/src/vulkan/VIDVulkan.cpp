...
#include "frameprofile.h"
  ...
  {
    return VIDVulkan::getInstance()->init();
  }
  ...
  else
  {
    ...
    if (resolutionMode != RES_NATIVE){
      ...
      case VK_SURFACE_TRANSFORM_ROTATE_180_BIT_KHR:
        viewportData = {deviceHeight - finalWidth - originx, deviceWidth - finalHeight - originy, finalWidth, finalHeight};
        break;
      ...
      case VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR:
        ...
        break;
      case VK_SURFACE_TRANSFORM_ROTATE_180_BIT_KHR:
        viewportData = {deviceHeight - renderWidth - originx, deviceWidth - renderHeight - originy, renderWidth, renderHeight};
        break;
      case VK_SURFACE_TRANSFORM_ROTATE_270_BIT_KHR:
        ...
        if (layers[i][j]->vertexSize > 0)
        {
          ...
          if (layers[i][j]->blendmode != 0 && fbRendered == false)
          {
            ...
            if (Vdp1External.disptoggle & 0x01)
            {
              ...
              {
                fbRender->drawWithDestAlphaMode(fixVdp2Regs, _command_buffers[fi], from, to, pre_rotate_mat);
              }
              else
              {
                fbRender->draw(fixVdp2Regs, _command_buffers[fi], from, to, pre_rotate_mat);
              }
            }
            from = to;
            fbRendered = true;
    ...
    if (Vdp1External.disptoggle & 0x01)
    {
      if (isDestinationAlpha)
      {
        fbRender->drawWithDestAlphaMode(fixVdp2Regs, _command_buffers[fi], from, 10, pre_rotate_mat);
      }
      else
      {
        fbRender->draw(fixVdp2Regs, _command_buffers[fi], from, 10, pre_rotate_mat);
      }
    }
    ...
    {
      fbRender->drawShadow(fixVdp2Regs, _command_buffers[fi], 0, 10, pre_rotate_mat);
    }
    ...
    if (resolutionMode != RES_NATIVE)
    {
      ...
      c.setScissor(0, 1, &scissor);
      vkCmdEndRenderPass(_command_buffers[fi]);
      blitSubRenderTarget(_command_buffers[fi], viewportData);
      NanovgVulkanSetDevices(device, this->getPhysicalDevice(), _renderer->getWindow()->GetVulkanRenderPass(), _command_buffers[fi]);
      OSDDisplayMessages(NULL, 0, 0);
    }
  }
...
int VIDVulkan::genPolygon(VdpPipeline **pipleLine, vdp2draw_struct *input, CharTexture *output, float *colors, TextureCache *c, int cash_flg)
{
  ...
  if (input->colornumber >= 3)
  {
    ...
    */
    ...
    else if (input->linescreen == 2)
    { // per line operation by HBLANK
    ...
    int rbg_type)
{
  ...
  if (input->colornumber >= 3)
  {
    ...
    if (input->lineTexture != 0)
    { // Line color blend screen blend is done in 2nd path
      prg = PG_VDP2_NOBLEND;
    }
...
void VIDVulkan::drawNBG0()
{
  ...
  if (fixVdp2Regs->CCCTL & 0x1)
  {
    ...
    { // Use Color calculation ratio
      if (info.specialcolormode != 0 && dest_alpha)
      { // Just currently not supported
...
void VIDVulkan::drawRotation_in(RBGDrawInfo *rbg)
{
  ...
  if (rbg->useb)
  {
    ...
    paraB.Xp = paraB.A * (paraB.Px - paraB.Cx) + paraB.B * (paraB.Py - paraB.Cy) + paraB.C * (paraB.Pz - paraB.Cz) + paraB.Cx + paraB.Mx;
    paraB.Yp = paraB.D * (paraB.Px - paraB.Cx) + paraB.E * (paraB.Py - paraB.Cy) + paraB.F * (paraB.Pz - paraB.Cz) + paraB.Cy + paraB.My;
  }
