...
u16 FASTCALL JapModemCs0ReadWord(SH2_struct *sh, u32 addr)
{
   NETLINK_LOG("%08X Cs0 word read. PC = %08X", addr, MSH2->regs.PC);
   return 0xFFA5;
}

u32 FASTCALL JapModemCs0ReadLong(SH2_struct *sh, u32 addr)
{
   NETLINK_LOG("%08X Cs0 long read. PC = %08X", addr, MSH2->regs.PC);
   return 0xFFA5FFA5;
}

u8 FASTCALL JapModemCs1ReadByte(SH2_struct *sh, u32 addr)
{   
   NETLINK_LOG("%08X Cs1 byte read. PC = %08X", addr, MSH2->regs.PC);
   return 0xFF;
}

u16 FASTCALL JapModemCs1ReadWord(SH2_struct *sh, u32 addr)
{
   NETLINK_LOG("%08X Cs1 word read. PC = %08X", addr, MSH2->regs.PC);
   return 0xFFFF;
}

u32 FASTCALL JapModemCs1ReadLong(SH2_struct *sh, u32 addr)
{
