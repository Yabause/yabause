...
"    if (all(lessThanEqual(dist(P-vec2(0.5), Quad[0], Quad[1]), vec2(0.5)))) {return 2u;}\n"
"    if (cmd[idx].type < "Stringify(LINE)") {\n"
"      if (all(lessThanEqual(dist(P-vec2(0.5), Quad[1], Quad[2]), vec2(0.5)))) {return 3u;}\n"
"      if (all(lessThanEqual(dist(P-vec2(0.5), Quad[2], Quad[3]), vec2(0.5)))) {return 4u;}\n"
"      if (all(lessThanEqual(dist(P-vec2(0.5), Quad[3], Quad[0]), vec2(0.5)))) {return 5u;}\n"
"    }\n"
// "  }\n"
"  return 0u;\n"
"}\n"
...
"}\n"
"uint getEnd(uint x, uint pos, cmdparameter_struct pixcmd) {\n"
" uint endCnt = 0;\n"
" for (uint i = 0; i<x; i++) {\n"
"   switch ((pixcmd.CMDPMOD >> 3) & 0x7u) {\n"
"     case 0:\n"
"     case 1: {\n"
"       uint dot = Vdp1RamReadByte(pixcmd.CMDSRCA * 8 + (pos+i)/2);\n"
"       if ((i & 0x1u) == 0u) dot = (dot>>4)&0xFu;\n"
"       else dot = (dot)&0xFu;\n"
"       if (dot == 0x0Fu) {\n"
"         endCnt++;\n"
"       }\n"
...
"     case 3:\n"
"     case 4: {\n"
"       if (Vdp1RamReadByte(pixcmd.CMDSRCA * 8 + (pos+i)) == 0xFFu) {\n"
"         endCnt++;\n"
"       }\n"
"       if (endCnt == 2) return endCnt;\n"
"     }\n"
...
"  discarded = false;\n"
"  if (!END) endCnt = getEnd(x, pos-x, pixcmd);\n"
"  if (endCnt >= 2u) {\n"
"    discarded = true;\n"
"    return vec4(0.0);\n"
"  }\n"
...
"      texcoord = vec2(float(texel.x/upscale.x-pixcmd.CMDXA+0.5)/float(pixcmd.w), float(texel.y/upscale.y-pixcmd.CMDYA+0.5)/float(pixcmd.h));\n"
"      gouraudcoord = texcoord;\n"
"      if ((pixcmd.flip & 0x1u) == 0x1u) texcoord.x = 1.0 - texcoord.x;\n" //invert horizontally
"      if ((pixcmd.flip & 0x2u) == 0x2u) texcoord.y = 1.0 - texcoord.y;\n" //invert vertically
#ifdef USE_VDP1_TEX
