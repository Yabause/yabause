...
void gdb_client_received(gdb_client * client, gdb_packet * packet)
{
   ...
   else if (packet->buffer[0] == 'm')
   {
      ...
      i = 0;
      while(i < len / 4) {
         u32 val = MappedMemoryReadLongNocache(MSH2, client->context, addr + 4 * i);
         sprintf(pos, "%08x", val);
         i += 1;
         pos += 8;
      }
      ...
      switch(len % 4) {
         case 2: {
            u16 val = MappedMemoryReadWordNocache(MSH2, client->context, addr + 4 * i);
            sprintf(pos, "%04x", val);
            break;
         }
         case 1: {
            u8 val = MappedMemoryReadByteNocache(MSH2, client->context, addr + 4 * i);
            sprintf(pos, "%02x", val);
            break;
         }
      }
